...
public interface AsioVisitor {
    ...
    class FSM {
        ...
        public static final Map<SelectionKey, SSLEngine> sslState = new WeakHashMap<>();
        /**
         * stores {InterestOps,{attachment}}
         */
        public static final Map<SelectionKey, Pair<Integer, Object>> sslGoal = new WeakHashMap<>();
        ...
        public static void delegateTasks(final Pair<SelectionKey, SSLEngine> state) throws InterruptedException {
            ...
            barrier.set(new CyclicBarrier(runnables.size(), () -> {
                try {
                    handShake(state);
                } catch (Exception e) {
                    e.printStackTrace();
        ...
        enum sslBacklog {
            ...
            public final Map<SelectionKey, ByteBuffer> per = new WeakHashMap<>();
    ...
    class Helper {
         ...
         * @throws Exception
         */
        public static void sslClient2(final String host, final int port, final Impl asioVisitor, final int clientOps) throws Exception {
            SocketChannel open = SocketChannel.open();
            open.configureBlocking(false);
            final InetSocketAddress remote = new InetSocketAddress(host, port);
            open.connect(remote);
            finishConnect(open, key -> {
        ...
        public static int write(SelectionKey key, ByteBuffer src) throws Exception {
            ...
            int write;
            if (null == sslEngine) {
                return ((SocketChannel) key.channel()).write(src);
            }
        ...
        public static int read(SelectionKey key, ByteBuffer toApp) throws Exception {
            ...
            int read;
            if (null == sslEngine) {
                return ((SocketChannel) key.channel()).read(toApp);
            }
        ...
        public static void finishWriteSeq(SelectionKey key, final Runnable success, final ByteBuffer... payload) {
            finishWriteSeq(key, key1 -> success.run(), payload);
        }
...
//          }
        ...
        public static <T extends F> T park(final F... then) {
            return (T) (F) key -> {
                key.interestOps(0);
                if (then.length > 0) {
                    then[0].apply(key);
        ...
        public static F isolate(final F onSuccess) {
            return key -> FSM.getExecutorService().submit((Callable<Void>) () -> {
                onSuccess.apply(key);
