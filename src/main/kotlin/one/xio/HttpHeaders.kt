...
enum class HttpHeaders {
    ...
    val header get()= URLDecoder.decode(name)
    val token get()= StandardCharsets.UTF_8.encode(header)
    val tokenLen   get()= token.limit()
    ...
    fun parse(slice: ByteBuffer): ByteBuffer {
        ...
        while ( isWhitespace(slice.get(slice.limit() - 1) as Int and 0xff  ))
    ...
    fun recognize(buffer: ByteBuffer): Boolean {
        ...
        var ret = false
        if (buffer[tokenLen + i] as Int and 0xff === ':'.toInt()) {
